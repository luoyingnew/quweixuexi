//
//  JJFunTrackHomeworkViewController.m
//  FunStudy
//
...
static NSString *funTrackHomeworkCellIdentifier = @"JJFunTrackHomeworkCellIdentifier";

@interface JJFunTrackHomeworkViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation JJFunTrackHomeworkViewController
...
- (void)viewDidLoad {
    ...
    [self setMjHeadFoot];
    [self.tableView registerClass:[JJFunTrackTestCell class] forCellReuseIdentifier:funTrackHomeworkCellIdentifier];
...
- (void)loadNewRequestWithDataList {
    ...
    NSDictionary *params = @{@"fun_user_id" : [User getUserInformation].fun_user_id, @"eh_type" : @1, @"page" : @(self.currentPage), @"pageSize" : @(self.pageSize)};
...
- (void)loadMoreRequestWithDataList {
    ...
    NSString *URL =[NSString stringWithFormat:@"%@%@", DEVELOP_BASE_URL, API_FOOTPRINT];
    NSDictionary *params = @{@"fun_user_id" : [User getUserInformation].fun_user_id, @"eh_type" : @1, @"page" : @(self.currentPage + 1)};
    [JJHud showStatus:nil];
    HFURLSessionTask *task = [HFNetWork postWithURL:URL params:params isCache:NO success:^(id response) {
        [JJHud dismiss];
        [self.tableView.mj_footer endRefreshing];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    JJFunTrackTestCell *cell= [tableView dequeueReusableCellWithIdentifier:funTrackHomeworkCellIdentifier forIndexPath:indexPath ];
