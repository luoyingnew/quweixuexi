//
//  JJBilingualResultViewController.m
//  FunStudy
//
...
@interface JJBilingualResultViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation JJBilingualResultViewController
...
//    if(self.modelArray.count == 0) return;
    ...
    for(int i = 0; i < self.bilingualResultModelArray.count; i++) {
        ...
        if([self.name isEqualToString:@"单词拼写结果"]) {
            if(i == 0) {
        ...
        } else if ([self.name isEqualToString:@"中英对照结果"]) {
            NSArray *seprateArray =  [model.myAnswerleftText componentsSeparatedByString:@"-"];
            NSString *string = nil;
            if(seprateArray.count > 1) {
                string = seprateArray[1];
            } else {
                string = @"X";
            }
            if(i == 0) {
                ...
                answer_index = [NSString stringWithFormat:@"%@_%@",answer_index,model.unit_id];
                answer_content = [NSString stringWithFormat:@"%@_%@",answer_content,string];
            }
        }
    }
    NSString *URL = nil;
