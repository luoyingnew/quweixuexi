...
@interface JJTransLateViewController ()
...
@property (nonatomic, strong) NSMutableArray<JJTranslateModel *> *modelArray;
...
@implementation JJTransLateViewController
...
//    
//    JJTranslateModel *model1 = [[JJTranslateModel alloc]init];
//    model1.question = @"妈妈配偶读书";
//    JJTranslateView *translateV1 = [JJTranslateView translateViewWithType:SingleCHToEn singleTranslateModel:model1];
//    
//    
//    JJTranslateModel *model2 = [[JJTranslateModel alloc]init];
//    model2.question = @"hdfayiu ywiueyrfiuwa";
//    JJTranslateView *translateV2 = [JJTranslateView translateViewWithType:SingleEnToCH singleTranslateModel:model2];
//    translateV1.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.scrollView.height);
//    [self.scrollView addSubview:translateV1];
//    
//    translateV2.frame = CGRectMake(SCREEN_WIDTH, 0, SCREEN_WIDTH, self.scrollView.height);
//    [self.scrollView addSubview:translateV2];
...
//        weakSelf(weakSelf);
        for(NSDictionary *dict in dataArray) {
            JJTranslateModel *translateModel = [JJTranslateModel modelWithDictionary:dict];
            translateModel.topic_title = self.topicModel.topic_title;
            JJTranslateView *translateView = [JJTranslateView translateViewWithSingleTranslateModel:translateModel];
            translateView.frame = CGRectMake(self.modelArray.count * SCREEN_WIDTH, 0, SCREEN_WIDTH, self.scrollView.height);
            [self.scrollView addSubview:translateView];
            [self.modelArray addObject:translateModel];
...
- (void)requestToSubmitHomework {
    ...
    for(int i = 0; i < self.modelArray.count; i++) {
        JJTranslateModel *model = self.modelArray[i];
        if(i == 0) {
