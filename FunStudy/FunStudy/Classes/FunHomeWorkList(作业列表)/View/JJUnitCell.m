...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if(self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.unitNameLabel.font = [UIFont boldSystemFontOfSize:15 * KWIDTH_IPHONE6_SCALE];
        self.unitNameLabel.textColor = RGBA(28, 168, 255, 1);
        self.unitNameLabel.text = @"单元名称";
        self.unitNameLabel.numberOfLines = 1;
        [self.contentView addSubview:self.unitNameLabel];
        [self.unitNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)setModel:(JJUnitModel *)model {
    _model = model;
    self.unitNameLabel.text = model.unit_title;
