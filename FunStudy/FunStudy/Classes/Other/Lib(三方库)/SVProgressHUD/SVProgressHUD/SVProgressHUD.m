...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        if ([UIFont respondsToSelector:@selector(preferredFontForTextStyle:)]) {
            ...
            _font = [UIFont systemFontOfSize:14.0f * KWIDTH_IPHONE6_SCALE];
        }
        ...
        _ringThickness = 2.0f * KWIDTH_IPHONE6_SCALE;
        _ringRadius = 18.0f * KWIDTH_IPHONE6_SCALE;
        _ringNoTextRadius = 24.0f * KWIDTH_IPHONE6_SCALE;
        
        _cornerRadius = 14.0f * KWIDTH_IPHONE6_SCALE;
...
- (void)updateHUDFrame {
    ...
    CGFloat hudWidth = 100.0f * KWIDTH_IPHONE6_SCALE;
    CGFloat hudHeight = 100.0f * KWIDTH_IPHONE6_SCALE;
    CGFloat stringHeightBuffer = 20.0f * KWIDTH_IPHONE6_SCALE;
    CGFloat stringAndContentHeightBuffer = 80.0f * KWIDTH_IPHONE6_SCALE;
    ...
    if(string) {
        CGSize constraintSize = CGSizeMake(200.0f * KWIDTH_IPHONE6_SCALE, 300.0f * KWIDTH_IPHONE6_SCALE);
        CGRect stringRect;
...
#pragma clang diagnostic ignored "-Wdeprecated"
                stringSize = [string sizeWithFont:self.statusLabel.font constrainedToSize:CGSizeMake(200.0f * KWIDTH_IPHONE6_SCALE, 300.0f * KWIDTH_IPHONE6_SCALE)];
#pragma clang diagnostic pop
        ...
        CGFloat labelRectY = (imageUsed || progressUsed) ? 68.0f*KWIDTH_IPHONE6_SCALE : 9.0f * KWIDTH_IPHONE6_SCALE;
        if(hudHeight > 100.0f * KWIDTH_IPHONE6_SCALE) {
            labelRect = CGRectMake(12.0f*KWIDTH_IPHONE6_SCALE, labelRectY, hudWidth, stringHeight);
            hudWidth += 24.0f * KWIDTH_IPHONE6_SCALE;
        } else {
            hudWidth += 24.0f * KWIDTH_IPHONE6_SCALE;
    ...
    if(string) {
        self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 36.0f * KWIDTH_IPHONE6_SCALE);
    } else {
	...
	if(string) {
        ...
        CGPoint center = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), 36.0f * KWIDTH_IPHONE6_SCALE);
        self.indefiniteAnimatedView.center = center;
        
        if(self.progress != SVProgressHUDUndefinedProgress) {
            self.backgroundRingView.center = self.ringView.center = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), 36.0f * KWIDTH_IPHONE6_SCALE);
        }
	} else {
        if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat) {
            SVIndefiniteAnimatedView *indefiniteAnimationView = (SVIndefiniteAnimatedView*)self.indefiniteAnimatedView;
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    if([self.hudView respondsToSelector:@selector(addMotionEffect:)]) {
        ...
        effectX.minimumRelativeValue = @(-10 * KWIDTH_IPHONE6_SCALE);//@(-SVProgressHUDParallaxDepthPoints);
        effectX.maximumRelativeValue = @(10 * KWIDTH_IPHONE6_SCALE);//@(SVProgressHUDParallaxDepthPoints);
        
        UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
        effectY.minimumRelativeValue = @(-10 * KWIDTH_IPHONE6_SCALE);//@(-SVProgressHUDParallaxDepthPoints);
        effectY.maximumRelativeValue = @(10 * KWIDTH_IPHONE6_SCALE);//@(SVProgressHUDParallaxDepthPoints);
...
- (UIImageView*)imageView {
    if(!_imageView) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 28.0f * KWIDTH_IPHONE6_SCALE, 28.0f * KWIDTH_IPHONE6_SCALE)];
    }
