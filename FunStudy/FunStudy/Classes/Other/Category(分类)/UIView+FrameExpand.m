...
//dashBorder
- (void)createDashLineBordersWithColor:(UIColor *)color withCornerRadius:(CGFloat)radius andWidth:(CGFloat)width {
    //把之前的虚线删除掉
    ...
    for(CALayer *layer in sublayers) {
        if([layer isMemberOfClass:[CAShapeLayer class]]) {
            ...
            if(shaLayer.lineWidth == width && shaLayer.lineDashPattern != nil) {
                deleteLayer = layer;
                break;
            }
        }
...
//    [self.layer.sublayers.lastObject removeFromSuperlayer];
    ...
    NSLog(@"%@",NSStringFromCGPoint(borderLayer.position));
