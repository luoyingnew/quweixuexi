...
- (void)internalConfig {
    ...
    button.frame = CGRectMake(self.frame.size.width - 42, 0, 35, 43.5);
    button.backgroundColor = [UIColor clearColor];
    button.titleLabel.font = [UIFont systemFontOfSize:16];
    [button setTitleColor:[UIColor colorWithRed:0/255.0 green:122/255.0 blue:255/255.0 alpha:1.0] forState:UIControlStateNormal];
...
- (void)showPickerWithProvinceName:(NSString *)provinceName cityName:(NSString *)cityName countyName:(NSString *)countyName {
    ...
    if (provinceName.length > 0) {
        [_firstAry enumerateObjectsUsingBlock:^(NSDictionary *provinceDic, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([provinceDic[@"-name"] isEqualToString:provinceName]) {
                ...
                _secondAry = provinceDic[@"city"];
                }else{
                    NSDictionary *dict = provinceDic[@"city"];
                    _secondAry = @[dict];
                }
            }
        }];
    ...
    [_secondAry enumerateObjectsUsingBlock:^(NSDictionary *cityDic, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([cityDic[@"-name"] isEqualToString:cityName]) {
            ...
            if ([cityDic[@"district"] isKindOfClass:[NSArray class]]) {
                _thirdAry = cityDic[@"district"];
            }
        }
    }];
    
    [_thirdAry enumerateObjectsUsingBlock:^(NSDictionary *countyDic, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([countyDic[@"-name"] isEqualToString:countyName]) {
...
- (NSString *)pickerView:(UIPickerView *)thePickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    if (component == 0) {
        ...
        if (row > 0) {
            DebugLog(@"row : %ld   second.count = %ld",row,_secondAry.count);
...
- (void)pickerView:(UIPickerView *)thePickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (component == 0) {
        ...
        if (row > 0) {
            ...
            //如果是数组
            if([dic[@"city"] isKindOfClass:[NSArray class]]){
                _secondAry = dic[@"city"];
            }else{
                NSDictionary *dict = dic[@"city"];
                _secondAry = @[dict];
            }
